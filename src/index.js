...
module.exports = Object.assign({}, tags, {
  updateElement
})
...
const eventExceptions = ['onClick', 'onSubmit']
...
function createElement (tagName, textOrPropsOrChild, ...otherChildren) {
  ...
  if (toType(textOrPropsOrChild) === 'object') {
    // handle props
    for (let name in textOrPropsOrChild) {
      ...
      } else if (eventExceptions.indexOf(name) > -1) {
        const eventName = name.slice(2).toLowerCase()
        el.addEventListener(eventName, textOrPropsOrChild[name])
      } else if (name.indexOf('data-') === 0) {
        setAttributes(el, {[name]: textOrPropsOrChild[name]})
      } else {
        console.warn(`${name} is not a valid property of a <${tagName}>`, { name, tagName, el })
      }
    }
  } else {
    // append text or child
